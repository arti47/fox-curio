...
|---|---|---|---|
| 2026-07-16 | **Fix: screen jumped to top on every button press.** Root cause: `router.render` always called `window.scrollTo(0,0)`, and every in-place action re-renders via `go(sameRoute)`, so any button (heart +/−, supply edit, favour, etc.) snapped you back to the top. Fix: `go` compares the target to the current route and only scrolls to top on an actual **tab change**; same-route re-renders keep the scroll position. SW → v0.25.0. | `npm test` **139/139**; headless @390px: scrolled to y=1015, clicked a supply −, y stayed 1015 (preserved); tab change reset to y=0; zero console errors | fox-curio-v0.25.0 |
| 2026-07-16 | **Inventory coins/books: tap-to-edit dialog (was unthemed white boxes).** The raw `<input type=number>`+Set on the Inventory card rendered unstyled white (they sat outside `.field`) and misaligned. Replaced with a clean row — icon + label + bold figure (`.res-value`) + a **✎ Edit** button that opens the themed `promptModal` to type the new amount (books still cap-clamped via `setResource`). No raw inputs on the card. SW → v0.24.0. | `npm test` **139/139**; headless @390px: 0 number inputs on card, 2 Edit buttons, modal input themed (not white), edit coins→777, books 9999→cap 500; dark screenshot clean; zero overflow, zero console errors | fox-curio-v0.24.0 |
| 2026-07-16 | **Manual inventory/resource edits (house aid, §8.8).** The Inventory card (Home) is now editable: **Coins** and **Books** number inputs + Set (books clamp to cap), **＋/−/Remove** on each supply, and **Remove one** per caught-fish name. Friends card gains **Reset favours** (`profiles.resetFavours` clears `favoursUsed`). Nothing was hand-editable before — resources only moved through game actions. Explicitly labelled a house aid, not a book rule. SW → v0.23.0. Harness +5. | `npm test` **139/139**; headless @390px: set coins 50→999, books 9999→cap 500, Candles −1→1 then Remove→0, Trout remove-one→1, Reset favours (used 1→0); zero overflow, zero console errors | fox-curio-v0.23.0 |
